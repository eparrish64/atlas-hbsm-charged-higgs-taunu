...
outfilename = "eff_mass_taujet_merged.cxx"
...
def plot_meff(fileName):
  ...
  ROOT.gROOT.LoadMacro("eff_mass_taujet.cxx")
  from ROOT import eff_mass_taujet
  ...
  for ii, xx in enumerate(x):
    ...
    statplus = 0.0
    statminus = 0.0
    ynom = eff_mass_taujet(int(xx+halfbin),0)
    #print(" xx = ",xx)
    #print(" int(xx+halfbin) = ",xx)
    
    for i, syst in enumerate(systs):
      #print(" i = ",i,"  syst = ",syst)
      y = eff_mass_taujet(int(xx+halfbin),syst) - ynom
      if y>0:
          statplus += y*y
      else:
          statminus += y*y
  ...
  with open(outfilename, "a") as cfile:
                ...
                for ii, xx in enumerate(x):
                      cfile.write("%0.3f, "%(eff_mass_taujet(int(xx+halfbin),index)))
                cfile.write("%0.3f \n"%(1.0))
                cfile.write("\t\t\t  }; \n")
...
if __name__=="__main__":
    ...
    with open(outfilename, "w") as cfile:
            ...
            cfile.write("//             To be used for the tau+jets channel!\n")
            cfile.write("//             in a the lookup table form (after compactification)\n")
            istring = "float eff_mass_taujet(float eff_m,int var){\n"
            cfile.write(istring)
