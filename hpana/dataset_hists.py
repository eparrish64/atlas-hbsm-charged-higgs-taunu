...
def dataset_hists(hist_worker,
        ...
        for var in fields:
            ...
            for category in categories:
                if hist_templates and var.name in hist_templates:
                    ...
                    if var.bins:
                        hist = ROOT.TH1F(
                            "syst_%s_category_%s_var_%s"%(variation, category.name, var.name), var.name, len(var.bins)-1, array.array("d", var.bins))
                    else:    
                        hist = ROOT.TH1F(
                            "syst_%scategory_%s_var_%s"%(variation, category.name, var.name), var.name, *var.binning)
    ...
    for fn in dataset.files:
        ...
        if systematic.styep=="TREE":
            tree_names = systematic.variations
        else:
            tree_names = ("NOMINAL",)
        ...
        for tree_name in tree_names:
            ...
            for n, category in enumerate(categories):
                ...
                # - - event weight
                if systematic.name in weights[category.name]:
                    eventweight = weights[category.name][systematic.name]
                else:
                    eventweight = weights[category.name]["NOMINAL"]
