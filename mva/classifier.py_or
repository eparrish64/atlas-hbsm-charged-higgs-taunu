...
def train_model(model, 
    ...
    #    log.info("Balancing training classes (under-sampling); signal events:{} | bkg events: {} , Trained model {}".format(s_df.shape[0], b_df.shape[0],model.name))
    #    b_df = b_df.sample(s_df.shape[0], replace=True)
    #    log.info("PO Balancing training classes (under-sampling); signal events:{} | bkg events: {} , Trained model {}".format(s_df.shape[0], b_df.shape[0],model.name))
    #    tr_df_ud = pd.concat([b_df, s_df], axis=0)
    #ak
    weight_sample = False
    b_df = tr_df[tr_df["class_label"]==0]
    s_df = tr_df[tr_df["class_label"]==1]
    log.info("Training classes; signal events:{} | bkg events: {} , Trained model {}".format(s_df.shape[0], b_df.shape[0],model.name))
    if balanced:
        log.info("Balancing training classes (under-sampling); signal events:{} | bkg events: {} , Trained model {}".format(s_df.shape[0], b_df.shape[0],model.name))
        b_df = b_df.sample(s_df.shape[0], replace=True)
        log.info("PO Balancing training classes (under-sampling); signal events:{} | bkg events: {} , Trained model {}".format(s_df.shape[0], b_df.shape[0],model.name))
