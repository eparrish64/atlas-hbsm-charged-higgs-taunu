    ...
    for sig in signals:
        ...
        for m_model in models:
            ...
            if not (masses[0] <= sig.mass <= masses[-1]):
                continue
            #print m_model.name, m_model.mass_range
            ...
            ##b1_arr = np.concatenate([b_tr_score])
            ##s1_arr = np.concatenate([s_tr_score])
        ...
        print sig.mass,mean_auc,err_mean_auc 
        ...
        if plot_roc:
            ...
            ##ak for i in range(0, len(fpr_grd)):
            ##ak    print fpr_grd[i],tpr_grd[i]
